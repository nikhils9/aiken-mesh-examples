import {
  ...
  lucid.selectWalletFromPrivateKey(await Deno.readTextFile("./beneficiary-lucid.sk"));
  const beneficiaryPublicKeyHash = await lucid.utils.getAddressDetails(await lucid.wallet.address()).paymentCredential.hash;
...
async function unlock(utxos, currentTime, { from, using }): Promise<TxHash> {
  ...
  const tx = await lucid
    ...
    .validFrom(currentTime - 30*1000) // .validFrom(currentTime - 1000*5*60)
    .validTo(laterTime)
